...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        discounts = [self.gamma ** t for t in indices]

        # 3) create a list where the entry at each index (t') is gamma^(t') * r_{t'}
        discounted_rewards = [d * rewards[t] for t, d in zip(indices, discounts)]
        ...
        list_of_discounted_returns = [sum_of_discounted_rewards for _ in range(T)]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(T):
            ...
            discounts = [self.gamma ** (t - start_time_index) for t in indices]
            ...
            discounted_rtg = [d * rewards[t] for t, d in zip(indices, discounts)]
